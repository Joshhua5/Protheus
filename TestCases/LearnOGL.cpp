...
int main() {
	Graphics::Window window("title", Vector2<int>(1500, 700));

	FileSystem fs;
    
 	auto cube = MeshLoader::LoadModel(fs.GetFile("scene/monkey.obj"));
	...
	while (!window.IsExitRequested()) {
		...
		light.position = ToVector3<float>(mousePos.Cast<float>());
		...
		if (window.keyboard().IsKeyDown(KEY::KEY_W))
			rotation.x += 0.1f;
		if (window.keyboard().IsKeyDown(KEY::KEY_D))
			rotation.y += 0.1f;
		if (window.keyboard().IsKeyDown(KEY::KEY_S))
			rotation.x -= 0.1f;
		if (window.keyboard().IsKeyDown(KEY::KEY_A))
			rotation.y -= 0.1f;
		if (window.keyboard().IsKeyDown(KEY::KEY_Q))
			rotation.z += 0.1f;
		if (window.keyboard().IsKeyDown(KEY::KEY_E))
			rotation.z -= 0.1f;
        if (window.keyboard().IsKeyDown(KEY::KEY_X))
            window.Close();
